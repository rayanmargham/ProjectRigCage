...
impl Plugin for PreloaderPlugin
{
    fn build(&self, app: &mut App) {
        app
            ...
            .add_loading_state(LoadingState::new(GameState::Preload).continue_to_state(GameState::Intro))
            ...
            .add_systems(
                (
                    ...
                    Clock,
                    take_care_of_clock
                ).in_set(OnUpdate(GameState::Preload))
            )
    ...
    }, ClockTimer(Timer::from_seconds(1.5, TimerMode::Repeating)))).with_children(|paren|
    {
        paren.spawn(
            (
                ...
                ClockTweens {alpha: Animator::new(tween2), scale: Animator::new(tween), idenifier: ClockGhost()}
            )
        );
    });
...
)
{
    ...
    for (tim, entit) in query.iter()
    {
        ...
        {
            let tween = Tween::new(EaseFunction::SineInOut, Duration::from_millis(1000), TransformScaleLens {start: Vec3::new(1.0, 1.0, 1.0), end: Vec3::new(2., 2., 2.)}).with_completed_event(0);
            let tween2 = Tween::new(EaseFunction::SineInOut, Duration::from_millis(900), SpriteColorLens {start: Color::rgba(1., 1., 1., 1.), end: Color::rgba(1., 1., 1., 0.)}).with_completed_event(0);
