...
{
    pub fn new(xml_path: String, atlas_handle: &Handle<TextureAtlas>, modify: &mut TextureAtlas) -> Option<Self>
    {
        ...
        match file
        {
            Ok(f) =>
            {
                ...
                match xml
                {
                    Ok(xml) => 
                    {
                        let sheet_bundle = SpriteSheetBundle {sprite: TextureAtlasSprite::new(0), texture_atlas: atlas_handle.clone(), ..default()};
                        let mut temp_offsets: Offsets = Offsets { offsets_vec: Vec::new() };
                        ...
                        // All done!
                        return Some(
                            SpriteXMLBundle { sprite_sheet: sheet_bundle, spritexml: SpriteXML { offsets: temp_offsets } }
                        );
                    },
                    Err(e) => 
                    {
