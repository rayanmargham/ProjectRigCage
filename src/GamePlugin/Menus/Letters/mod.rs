...
impl LetterBundle {
    pub fn new(
        ...
        preloaded: &Res<PreloadFunkinAssets>,
        texture_atlases: &mut ResMut<Assets<TextureAtlas>>,
        position: Transform,
    ) -> Option<Entity> {
        let bfs = texture_atlases.add(TextureAtlas::new_empty(
            preloaded.alphabet.clone(),
            Vec2::new(1024.0, 695.0),
        ));
        ...
        for c in text.chars() {
            if idx == 0 {
                entit = commands
                    ...
                    .with_children(|pare| {
                        let mut xml_bundle = SpriteXMLBundle::new(
                            "assets/images/alphabet.xml".to_string(),
                            &bfs,
                            bf,
                        )
                        .unwrap();
                        ...
                        xml_bundle
                            .spritexml
                            .add_anim_from_prefix(anim.clone(), true, 24)
                            .unwrap();
                        xml_bundle
                            .spritexml
                            .set_anim(anim, &mut xml_bundle.sprite_sheet.sprite, true)
                            .unwrap();
                        xml_bundle.spritexml.apply_offsets(
                            &xml_bundle.sprite_sheet.sprite,
                            &mut xml_bundle.sprite_sheet.transform,
                        );
...
impl LetterBundle3D {
    pub fn new(
        ...
        let bfs = sprite_params.atlases.add(TextureAtlas::new_empty(
            preloaded.alphabet.clone(),
            Vec2::new(1024.0, 695.0),
        ));
        let mut idx = 0;
        ...
        if !align_center {
            for c in text.chars() {
                if idx == 0 {
                    entit = commands
                        ...
                        .with_children(|pare| {
                            let mut xml_bundle = SpriteXMLBundle3D::new(
                                "assets/images/alphabet.xml".to_string(),
                                &bfs,
                                sprite_params,
                                true,
                            ...
                            xml_bundle
                                .spritexml
                                .set_anim(anim, &mut xml_bundle.sprite_sheet.params, true)
                                .unwrap();
                            xml_bundle.spritexml.apply_offsets(
                                &xml_bundle.sprite_sheet.params,
                                &mut xml_bundle.sprite_sheet.pbr.transform,
                            );
            ...
            for c in text.chars() {
                ...
                if idx == 0 {
                    entit = commands
                        .spawn((SpatialBundle { ..default() }, Name::new(text.clone())))
                        .with_children(|pare| {
                            let mut xml_bundle = SpriteXMLBundle3D::new(
                                "assets/images/alphabet.xml".to_string(),
                                &bfs,
