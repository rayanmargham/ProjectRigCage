...
use crate::GamePlugin::{Gameplay::Conductor::{self, ConductorEvents, SongHandle}, Core::SpriteXML::{SpriteXMLBundle3D, SpriteXML3D}, GameStates::GameState, PreLoader::PreloadFunkinAssets};
...
pub fn title_init(asset_server: Res<AssetServer>, mut commands: Commands, mut sprite_params: Sprite3dParams, funkin: Res<FunkinAssets>)
{
    ...
    let efs = sprite_params.atlases.add(TextureAtlas::new_empty(funkin.pressenter.clone(), Vec2::new(4096.0, 4096.0)));
    ...
    // enter button
    let mut enter = SpriteXMLBundle3D::new("assets/images/titleEnter.xml".to_string(), &efs, &mut sprite_params, true).unwrap();
    enter.spritexml.add_anim_from_prefix("Press Enter to Begin".to_string(), true, 24).unwrap();
    enter.spritexml.add_anim_from_prefix("ENTER PRESSED".to_string(), true, 24).unwrap();
    enter.sprite_sheet.pbr.transform.translation = Vec3::new(1.25, -1.4, -4.0);
    enter.sprite_sheet.pbr.transform = enter.sprite_sheet.pbr.transform.with_scale(Vec3::new(0.5, 0.5, 0.5));
    enter.spritexml.set_anim("Press Enter to Begin".to_string(), &mut enter.sprite_sheet.params, true).unwrap();
    enter.spritexml.apply_offsets(&enter.sprite_sheet.params, &mut enter.sprite_sheet.pbr.transform);
...
pub fn bye_flash(mut commands: Commands, mut reader: EventReader<TweenCompleted>, query: Query<Entity, With<SpriteXML3D>>, mut next_state: ResMut<NextState<GameState>>)
{
...
pub fn beat_heat(mut commands: Commands, mut reader: EventReader<ConductorEvents>, mut query: Query<&mut SpriteXML3D, With<Logo>>, mut querygf: Query<(&mut AtlasSprite3dComponent, &mut SpriteXML3D, &mut GF), (With<GF>, Without<Logo>)>, input: Res<Input<KeyCode>>, audio: Res<Audio>, preload: Res<PreloadFunkinAssets>, mut queryenter: Query<(&mut AtlasSprite3dComponent, &mut SpriteXML3D, &mut EnterButton), (With<EnterButton>, Without<Logo>, Without<GF>)>, querycamera: Query<Entity, With<Camera3d>>)
{
    for ev in reader.iter()
    {
        match ev {
    ...
    if input.just_pressed(KeyCode::Return)
    {
        for (mut sprite, mut xml, mut button) in queryenter.iter_mut()
        {
            ...
            {
                let tween = Tween::new(EaseMethod::Linear, Duration::from_secs(1), SpriteColorLens
                {
                    start: Color::rgba(1.0, 1.0, 1.0, 1.0),
                    end: Color::rgba(1.0, 1.0, 1.0, 0.0)
                }).with_completed_event(0);
                ...
                xml.set_anim("ENTER PRESSED".to_string(), &mut sprite, false).unwrap();
                xml.reset_anim_idx().unwrap();
                commands.spawn((SpriteBundle
                    {
                        transform: Transform::from_xyz(0.0, 0.0, 0.0).with_scale(Vec3::new(1280.0, 720.0, 2.0)),
                ...
                {
                    let tween2 = Tween::new(EaseFunction::QuinticIn, Duration::from_secs_f64(2.5), TransformPositionLens
                    {
